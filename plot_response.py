...
data = [[float(line)  for line in open("response_for_expt%d_t%d" %(i,cs.TIME_LIMIT))] for i in range(cs.NUM_EXPTS)] 
...
pylab.hold(True)
fig.suptitle('Response y after t = %d, average of %d'%(cs.TIME_LIMIT,cs.NUM_EXPTS), fontsize=14)
#pylab.legend(loc='upper left',prop={'size':10})
pylab.savefig("response_t%d_mean_results.png"%(cs.TIME_LIMIT))
